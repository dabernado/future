...
instance Show FutureType where
  ...
  show (FuncT args Nothing return) = "(:Function (" ++ unwords (map show args) ++
                                    ") " ++ show return ++ ")"
  show (FuncT args (Just vararg) return) = "(:Function (" ++ unwords (map show args) ++
                                          " . " ++ show vararg ++
                                          ") " ++ show return ++ ")"
  show (CustomT t []) = ":" ++ t
